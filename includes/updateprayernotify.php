...
function updateprayernotify ($prayer_ID, $prayer_name, $prayer_answered, $prayer_title, $update_text){
	...
	while($prayernotifyrow = @mysql_fetch_assoc($prayernotifyresult)) {
		...
		$praymailsubject = "Updated Prayer Request from " . $prayer_name . " \n..";
		...
		$praymailmessage = "SQL Query for Prayer Updates failed to find prayer update requests - check updateprayernotify.php for details. There are " . $prayernotifyresultcount . " followers (" . $praymailtest . ") for prayer " . $prayer_ID . ".";
